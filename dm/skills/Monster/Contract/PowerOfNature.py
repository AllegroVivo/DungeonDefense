...
__all__ = ("PowerOfNature",)

################################################################################
class PowerOfNature(MonsterSkill):
    ...
    def __init__(self, state: DMGame, parent: DMUnit = None):
        ...
        super().__init__(
            ...
            description=(
                "Poison applied all monsters are converted to Regeneration, "
                "and Poison applied to enemy is doubled."
            ),
            rank=10,
    ...
    def notify(self, ctx: StatusApplicationContext) -> None:
        ...
        if ctx.status.name == "Poison":
            if isinstance(ctx.target, DMMonster):
                ctx.will_fail = True
                ctx.target.add_status("Regeneration", ctx.status.stacks, self)
